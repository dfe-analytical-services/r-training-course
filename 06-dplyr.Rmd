...
```r 
exercise_3 <- swfc_headcount %>% dplyr::select()
```
...
renamed_data_example <- data %>% 
                          rename(name_we_want = current_variable_name)
```

ii. If I wanted to rename the `la_number` variable to be called `la_num`, I'd run:

```r 
renamed_data_1 <- swfc_headcount %>% dplyr::rename(la_num = la_number)
```
...
```r
renamed_data_2 <- swfc_headcount %>% dplyr::rename(la_num = la_number,
                                                   school_name_at_2018 = school_name)
```
...
```r 
exercise_4 <- swfc_headcount %>% rename()
```
...
```r 
distinct_data_1 <- swfc_headcount %>% dplyr::distinct(region)
```
...
```r 
distinct_data_2 <- swfc_headcount %>% dplyr::distinct(region, la_name)
```
...
**Note: In the above example, I turned made sure to treat the teacher_count variable as a numeric value, why is this?**
...
```r
exercise_6 <- swfc_headcount %>% dplyr::arrange(as.numeric(as.character()))
```
...
```r
filtered_data_example_1 <- swfc_headcount %>% dplyr::filter(la_name == "Wigan")
```

ii. If I wanted to get all the data for all local authorities other than Wigan, I could use

```r
filtered_data_example_2 <- swfc_headcount %>% dplyr::filter(la_name != "Wigan")
```

iii. If I wanted to get all the data for two local authorities I could use

```r
filtered_data_example_3 <- swfc_headcount %>% dplyr::filter(la_name %in% c("Wigan", "Darlington"))
```

iv. If I wanted to filter data on multiple conditions, e.g. on `la_name` and `school_type`, I could use

```r
filtered_data_example_4 <- swfc_headcount %>% dplyr::filter(la_name == "Wigan" & school_type == "LA maintained schools")
```

v. If I wanted to filter the data to find schools with more than 20 teachers, 

```r
filtered_data_example_5 <- swfc_headcount %>% dplyr::filter(as.numeric(teacher_count) > 20)
```
